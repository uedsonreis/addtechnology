...
public class InferenceManager implements Serializable {
	...
	public String getRelationPossibilitiesString(Relation relation) {
		String temp="";
		
		Iterator it = relationPossibilitiesMap.values().iterator();
		
		while(it.hasNext()){
		 Integer number = (Integer)it.next(); 	
		 
		 	if(number == Relation.BIDIRECIONAL_COMPOSITION){
		 		temp+= " BIDIRECIONAL_COMPOSITION ";
		 	}
		 
		 	if(number == Relation.BIDIRECIONAL_COMPOSITION_HAS_BELONGS_TO ){
		 		temp += " BIDIRECIONAL_COMPOSITION_HAS_BELONGS_TO "; 
		 	}
		 
		 	if(number == Relation.EQUIVALENCE){
		 		temp += " EQUIVALENCE "; 
		 	}
		 
		 	if(number == Relation.INHERITANCE){
		 		temp += " INHERITANCE "; 
		 	}
		 
		 	if(number == Relation.SYMMETRIC_COMPOSITION){
		 		temp += " SYMMETRIC_COMPOSITION ";
		 	}
		 
		 	if(number == Relation.UNIDIRECIONAL_COMPOSITION){
		 		temp += " UNIDIRECIONAL_COMPOSITION ";
